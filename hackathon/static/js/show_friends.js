...
function updateUserList() {
    $.getJSON('/chat/api/v1/relationship', function (data) {
        userList.children('.user').remove();
        for (let i = 0; i < data.length; i++) {
            for(let j=0;j<data[i]['relationships'].length;++j){
                const userItem = `<a class="list-group-item user">${data[i]['relationships'][j]['nickname']}</a>`;
...
function setCurrentUser(username){
    $.getJSON('/chat/api/v1/Signup', function (data) {
        for(let i=0;i<data.length;++i){
            if(data[i]['user']==username){
                currentUser=data[i]['nickname'];   
            }
        }
    });
}
