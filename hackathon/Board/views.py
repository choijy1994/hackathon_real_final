...
def post_read(request):
    posts = Post.objects.filter(Confirm=False).order_by('-pub_date')
...
def post_search(request):
    ...
    searched_post.exclude(Confirm=True)
    nick = Signup.objects.values('user', 'nickname')
...
def post_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.user = Signup.objects.get(user=request.user)

 
            post.nation = request.POST.get('nation_post', False)
            post.lat = request.POST.get('lat_post', False)
            post.lng = request.POST.get('lng_post', False)
            post.spot = request.POST.get('attraction_post', False)

            
            
            post.nation = request.POST['nation']
            post.lat = request.POST['lat']
            post.lng = request.POST['lng']
            post.spot = request.POST['attraction']
...
def home_create(request):
    lat = request.GET.get('lat_post')
    lng = request.GET.get('lng_post')
    nation = request.GET.get('nation_post')
    attraction = request.GET.get('attraction_post')
    address = request.GET.get('address_post')
    form = NewPost()
    content = {
        'form':form,
        'lat':float(lat),
    ...
    print(content)
    return render(request, 'newpost.html', content)


def search_marker(request):
    lat = request.GET.get('lat_')
    lng = request.GET.get('lng_')
    searched_post = Post.objects.filter(Q(lat=lat) | Q(lng=lng)).distinct()
    nick = Signup.objects.values('user', 'nickname')
    paginator = Paginator(searched_post, 5)
    page = request.GET.get('page')
    searched_post = paginator.get_page(page)
...
def mypost(request):
    ...
    #print(name)
    posts = Post.objects.filter(user_id=name).order_by('-pub_date')
...
def myfriend(request, pk):
    ...
    Usr = Signup.objects.get(user = request.user)
    participant = participants.objects.filter(event=post).exclude(participate=Usr)
    context = {
        'post':post,
        'applicant':applicant,
...
def n_america(request):
    n_america = Country.objects.filter(cont__icontains="북아메리카")
    context = {
        'n_america' : n_america,
    }
...
def s_america(request):
    s_america = Country.objects.filter(cont__icontains="남아메리카")
    context = {
        's_america' : s_america,
    }
...
def aust(request):
    aust = Country.objects.filter(cont__icontains="오세아니아")
    context = {
        'aust' : aust,
    }
    return render(request, 'aust.html', context)

def africa(request):
    africa = Country.objects.filter(cont__icontains="아프리카")
    context = {
        'africa' : africa,
    }
...
def get_nation(request, name):
    ...
    return render(request, 'findcont.html', context)
